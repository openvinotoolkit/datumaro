...
from datumaro.plugins.data_formats.datumaro.base import DefaultReader
    ...
    def _load(self):
        ...
        for path in self._paths:
            with pa.ipc.open_stream(path) as reader:
                ...
                infos.append(DefaultReader._load_infos({"infos": _infos}))

                _categories, _ = DictMapper.backward(
                    schema.metadata.get(b"categories", b"\x00\x00\x00\x00")
                )
                categories.append(DefaultReader._load_categories({"categories": _categories}))
