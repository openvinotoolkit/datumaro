...
import grpc
import numpy as np
from client_utils import prepare_certs, print_statistics
...
parser = argparse.ArgumentParser(
    description="Demo for face detection requests via TFS gRPC API analyses input images "
...
parser.add_argument(
    ...
    default="../../common/static/images/people",
)
parser.add_argument(
    "--output_dir",
    required=False,
    help="Directory for storing images with detection results",
    default="results",
)
parser.add_argument(
    "--batch_size",
    required=False,
    help="How many images should be grouped in one batch",
    default=1,
    type=int,
)
parser.add_argument(
...
parser.add_argument(
    "--grpc_port", required=False, default=9000, help="Specify port to grpc service. default: 9000"
)
parser.add_argument(
    "--model_name", required=False, default="face-detection", help="Specify the model name"
)
parser.add_argument(
    "--tls", default=False, action="store_true", help="use TLS communication with gRPC endpoint"
)
...
if args.get("tls"):
    ...
    creds = grpc.ssl_channel_credentials(
        root_certificates=server_ca_cert, private_key=client_key, certificate_chain=client_cert
    )
...
for x in range(0, imgs.shape[0] - batch_size + 1, batch_size):
    ...
    for y in range(0, img.shape[0]):  # iterate over responses from all images in the batch
        ...
        for i in range(
            ...
            if (
                detection[0, 0, 2] > 0.5 and int(detection[0, 0, 0]) == y
                ...
                img_out = cv2.rectangle(
                    cv2.UMat(img_out), (x_min, y_min), (x_max, y_max), (0, 0, 255), 1
                )
                # draw each detected box on the input image
        print(
            "saving result to",
            os.path.join(args["output_dir"], str(iteration) + "_" + str(y) + ".jpg"),
        )
        cv2.imwrite(
            os.path.join(args["output_dir"], str(iteration) + "_" + str(y) + ".jpg"), img_out
        )

    print(
        "Iteration {}; Processing time: {:.2f} ms; speed {:.2f} fps".format(
            iteration,
