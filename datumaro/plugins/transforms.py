...
class RemapLabels(Transform, CliPlugin):
    ...
    def _make_label_id_map(self, src_label_cat, label_mapping, default_action):
        ...
        id_mapping = {}
        if src_label_cat:
            dst_label_cat.attributes=src_label_cat.attributes
        ...
        src_label = None
        dst_label = label_mapping.get(src_label)
        if dst_label:
            dst_index = dst_label_cat.find(dst_label)[0]
            if dst_index is None:
                dst_index = dst_label_cat.add(dst_label)
...
class AttrToLabelAttr(Transform, CliPlugin):
    @classmethod
    def build_cmdline_parser(cls, **kwargs):
        parser = super().build_cmdline_parser(**kwargs)
        parser.add_argument('-e', '--label',
            help="Label for moving attributes.")
        return parser
...
class LabelAttrToAttr(Transform, CliPlugin):
    @classmethod
    def build_cmdline_parser(cls, **kwargs):
        parser = super().build_cmdline_parser(**kwargs)
        parser.add_argument('-e', '--label',
    ...
    def __init__(self, extractor, label):
        ...
        if self._label == None:
            raise Exception("Label %s isn't in Label Categories" % label)
