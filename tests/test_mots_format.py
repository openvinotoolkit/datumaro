...
class MotsPngConverterTest(TestCase):
    ...
    def test_can_save_masks(self):
        source = Dataset.from_iterable([
            DatasetItem(id=1, subset='a', media=Image(data=np.ones((5, 1))),
                annotations=[
            ...
            DatasetItem(id=2, subset='a', media=Image(data=np.ones((5, 1))),
                annotations=[
                    Mask(np.array([[1, 0, 0, 0, 0]]), label=3,
                        attributes={'track_id': 2}),
                ]
            ),
            DatasetItem(id=3, subset='b', media=Image(data=np.ones((5, 1))),
                annotations=[
    ...
    def test_can_save_and_load_with_no_save_media(self):
        ...
        with TestDir() as test_dir:
            self._test_save_and_load(source_dataset,
                partial(MotsPngConverter.convert, save_media=False),
                test_dir)
    ...
    def test_can_save_dataset_with_cyrillic_and_spaces_in_filename(self):
        source = Dataset.from_iterable([
            DatasetItem(id='кириллица с пробелом', subset='a',
                media=Image(data=np.ones((5, 1))), annotations=[
    ...
    def test_can_save_and_load_image_with_arbitrary_extension(self):
        expected = Dataset.from_iterable([
            ...
            DatasetItem('a/b/c/2', media=Image(
...
class MotsImporterTest(TestCase):
    ...
    def test_can_import(self):
        target = Dataset.from_iterable([
            DatasetItem(id=1, subset='train', media=Image(data=np.ones((5, 1))),
                annotations=[
                    Mask(np.array([[0, 0, 0, 1, 0]]), label=3,
                        attributes={'track_id': 1}),
                    Mask(np.array([[0, 0, 1, 0, 0]]), label=2,
            ...
            DatasetItem(id=2, subset='train', media=Image(data=np.ones((5, 1))),
                annotations=[
                    Mask(np.array([[1, 0, 0, 0, 0]]), label=3,
                        attributes={'track_id': 2}),
                ]
            ),
            DatasetItem(id=3, subset='val', media=Image(data=np.ones((5, 1))),
                annotations=[
                    Mask(np.array([[0, 1, 0, 0, 0]]), label=0,
                        attributes={'track_id': 1}),
                ]
