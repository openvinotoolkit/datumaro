...
class PruneTest(TestCase):
    ...
    @scoped
    def test_can_prune_dataset_w_target(self):
    ...
    @mark_requirement(Requirements.DATUM_GENERAL_REQ)
    @scoped
    def test_can_prune_dataset_wo_target(self):
        test_dir = scope_add(TestDir())
        proj_dir = osp.join(test_dir, "proj")
        dataset_url = osp.join(test_dir, "dataset")
        ...
        run(self, "project", "create", "-o", proj_dir)
        run(self, "project", "import", "-p", proj_dir, "-f", "datumaro", dataset_url)
        run(self, "prune", "-m", "random", "-r", "0.5", "-p", proj_dir)
    ...
    @mark_requirement(Requirements.DATUM_GENERAL_REQ)
    @scoped
    def test_can_prune_wo_overwrite(self):
        test_dir = scope_add(TestDir())
        proj_dir = osp.join(test_dir, "proj")
        dataset_url = osp.join(test_dir, "dataset")
        ...
        run(self, "project", "create", "-o", proj_dir)
        run(self, "project", "import", "-p", proj_dir, "-f", "datumaro", dataset_url)
        run(self, "prune", "-m", "random", "-r", "0.5", "-p", proj_dir, "-o", dst_dir)
