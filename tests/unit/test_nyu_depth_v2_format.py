    ...
    def test_can_import(self):
        expected_dataset = Dataset.from_iterable(
            [
                DatasetItem(
                    id="1",
                    media=Image.from_data(data=np.ones((6, 4, 3))),
                    annotations=[DepthAnnotation(Image.from_data(data=np.ones((6, 4))))],
                ),
                DatasetItem(
                    id="2",
                    media=Image.from_data(data=np.ones((4, 3, 3))),
                    annotations=[DepthAnnotation(Image.from_data(data=np.ones((4, 3))))],
                ),
            ]
        )
