...
class WiderFaceFormatTest(TestCase):
    ...
    def test_can_save_and_load(self):
        source_dataset = Dataset.from_iterable(
            [
                ...
                DatasetItem(id="4", subset="val", media=Image.from_data(data=np.ones((8, 8, 3)))),
            ],
    ...
    def test_can_save_and_load_image_with_arbitrary_extension(self):
        dataset = Dataset.from_iterable(
            [
                DatasetItem("q/1", media=Image.from_data(data=np.zeros((4, 3, 3)), ext=".JPEG")),
                DatasetItem("a/b/c/2", media=Image.from_data(data=np.zeros((3, 4, 3)), ext=".bmp")),
            ],
            categories=[],
        )
