...
class DatumaroBinaryExporterTest:
    ...
    #     test_dir = get_test_asset_path("datumaro_binary_format")
    #     detected_formats = Environment().detect_dataset(test_dir)
    #     self.assertEqual([DatumaroBinaryImporter.NAME], detected_formats)

    @mark_requirement(Requirements.DATUM_GENERAL_REQ)
    def test_can_detect(self, fxt_test_datumaro_format_dataset: Dataset):
        with TestDir() as test_dir:
            DatumaroBinaryExporter.convert(self.fxt_test_datumaro_format_dataset, save_dir=test_dir)
