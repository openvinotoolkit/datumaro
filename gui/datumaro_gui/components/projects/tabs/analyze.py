...
from ..dashboard import Chart, ChartWithTab, Dashboard, DataGrid, DatasetInfoBox
...
def get_tab_data_point_dist_in_label(val_report):
    tabs = []
    keys = ["mean", "median"]  # , "stdev", "min", "max", "median"]
...
#         })
#         area_dist_total += item["count"]
# #def get_grid_data_point_dist_in_attr
# #def get_grid_data_point_dist_in_dataset_item
...
def main():
    ...
    if val_cls:
        ...
        # cls_label_dist = get_tab_data_for_val_label_dist(val_cls)
        # cls_attr_dist = get_tab_data_for_val_attr_dist(val_cls)
    if val_det:
        det_summary = get_validation_summary(val_det)
        det_anomaly_info = get_anomaly_info(val_det)
        # det_label_dist = get_tab_data_for_val_label_dist(val_det)
        # det_attr_dist = get_tab_data_for_val_attr_dist(val_det)
    if val_seg:
        seg_summary = get_validation_summary(val_seg)
        seg_anomaly_info = get_anomaly_info(val_seg)
        # seg_label_dist = get_tab_data_for_val_label_dist(val_seg)
        # seg_attr_dist = get_tab_data_for_val_attr_dist(val_seg)
