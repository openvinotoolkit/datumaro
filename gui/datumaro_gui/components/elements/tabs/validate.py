...
def main():
    ...
    with elements("general"):
        ...
        selected_task = st.selectbox(
            "Select a task for validation of dataset 1:", tasks, index=tasks.index(state["task_1"])
        )
        ...
        ## Dataset 2
        selected_task = st.selectbox(
            "Select a task for validation of dataset 2:", tasks, index=tasks.index(state["task_2"])
        )
        ...
        w = SimpleNamespace(
            ...
            data_grid_1=DataGrid(board, 3, 0, 6, 12, minH=4),
            data_grid_2=DataGrid(board, 3, 0, 6, 12, minH=4),
        )

        with w.dashboard(rowHeight=50):
            w.val_info_1(val_info_dict_1)
            w.val_info_2(val_info_dict_2)
            w.data_grid(data=anomaly_info_1, grid_name="Validation report of Dataset 1")
            w.data_grid(data=anomaly_info_2, grid_name="Validation report of Dataset 2")
