...
def main():
    ...
    with elements("merge"):
        ...
        if "data_helper_merged" in state:
            ...
            split_btn = col2.button("Do Split", use_container_width=True, key="split_btn_merged")

            if add_subset_btn:
                state["subset_merged"] += 1

            name, ratio = c1.columns(2)
            splits = []
            ...
            if split_btn:
                dataset = merged_data_helper.transform("random_split", splits=splits)
                state["subset_merged"] = 1
