...
``` bash
datum convert -if coco -i <path/to/dataset> -o <output/dir>
```

Extra options for converting from the COCO format:
...
``` bash
datum convert -if coco_stuff -i <path/to/dataset> -o <output/dir> -- -r <relpath/to/stuff.json>
```
...
You can convert a COCO dataset to other dataset formats using CLI:
...
You can convert a dataset to COCO format using CLI:
...
```bash
datum convert -if coco -i <path/to/dataset> -f coco -o <output/dir> -- --tasks instances,stuff
```
...
```bash
datum convert -if coco_panoptic -i ./COCO/annotations/panoptic_val2017.json \
    -f voc -o output_dir -- --save-media
datum stats ./COCO/annotations/panoptic_val2017.json
```
...
``` bash
datum convert --input-format roboflow_coco --input-path <path/to/dataset> \
