...
An Open Images dataset can be converted in the following way:

``` bash
datum convert --input-format open_images --input-path <path/to/dataset> \
...
There are several ways to convert a dataset to the Open Images format:
...
```bash
datum convert -if open_images -i ./open-images-dataset/ -f cvat -o <output/dir> -- --save-media
```
